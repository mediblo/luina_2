...
class RiotCog(commands.Cog):
    ...
    @app_commands.command(name="전적", description="해당 유저의 최근 10판을 확인합니다.") # 전적
    @app_commands.describe(nickname="태그 포함")
    @app_commands.choices(
        mode=[ # 신속, 일반, 솔랭, 자랭, 칼바람, 아레나
            app_commands.Choice(name="모두", value=-1),
            ...
            app_commands.Choice(name="칼바람", value=2400),
            app_commands.Choice(name="아레나", value=1700),
        ] 
    )
    async def match_log(self, interaction: discord.interactions, nickname:str, mode:int = -1):
        ...
        for match_id in matchs:
            ...
            m_data['queueId'] = api_data['info']['queueId'] # 게임 모드
